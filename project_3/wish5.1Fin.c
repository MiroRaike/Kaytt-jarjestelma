...
// To avoid copying the whole logic from inputs and file reading I've decided to just add paths as a global variable,
char **paths;
int pathAmount = 1;
...
int check_commands(char *line){
    ...
    // Take out \n because otherwise path command will not work properly
    token = strtok_r(line, "\n", &ptr);
